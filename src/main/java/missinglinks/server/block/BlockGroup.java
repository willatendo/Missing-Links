...
public class BlockGroup<T extends Block> {
	...
	public static <T extends StringRepresentable> List<BlockGroup> makeStairsSlabWallForEnumValues(String name, T[] enumValues) {
		...
		for (int i = 0; i < enumValues.length; i++) {
			...
			}).blockstate((block, provider) -> provider.stairsBlock(block.get(), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()))).tag(BlockTags.STAIRS, BlockTags.MINEABLE_WITH_PICKAXE).simpleItem().register(), REGISTRATE.block(enumValues[i].getSerializedName() + "_" + name + "_slab", SlabBlock::new).properties(p -> p.copy(base)).recipe((block, provider) -> {
			...
			}).blockstate((block, provider) -> provider.slabBlock(block.get(), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()))).tag(BlockTags.SLABS, BlockTags.MINEABLE_WITH_PICKAXE).simpleItem().register(), REGISTRATE.block(enumValues[i].getSerializedName() + "_" + name + "_wall", WallBlock::new).properties(p -> p.copy(base)).recipe((block, provider) -> {
	...
	public static BlockGroup makeButtonPressurePlate(Block base) {
		return new BlockGroup(new BlockEntry[] { REGISTRATE.block(base.getRegistryName().getPath().replace("_block", "").replace("bricks", "brick") + "_pressure_plate", properties -> new PressurePlateBlock(Sensitivity.MOBS, properties)).properties(p -> p.copy(base).noCollission().noOcclusion()).recipe((block, provider) -> {
			ShapedRecipeBuilder.shaped(block.get()).pattern("##").define('#', base).unlockedBy("has_block", RecipeProvider.has(base)).save(provider);
		}).blockstate((block, provider) -> provider.pressurePlateBlock(block.get(), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()))).tag(BlockTags.PRESSURE_PLATES, BlockTags.MINEABLE_WITH_PICKAXE).item().tab(() -> CreativeModeTab.TAB_REDSTONE).build().register(), REGISTRATE.block(base.getRegistryName().getPath().replace("_block", "").replace("bricks", "brick") + "_button", StoneButtonBlock::new).properties(p -> p.copy(base)).recipe((block, provider) -> {
		...
		}).blockstate((block, provider) -> provider.buttonBlock(block.get(), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()))).tag(BlockTags.BUTTONS, BlockTags.MINEABLE_WITH_PICKAXE).item().model((item, provider) -> provider.buttonInventory(item.getName(), new ResourceLocation(base.getRegistryName().getNamespace(), "block/" + base.getRegistryName().getPath()))).tab(() -> CreativeModeTab.TAB_REDSTONE).build().register() });
	}
}
